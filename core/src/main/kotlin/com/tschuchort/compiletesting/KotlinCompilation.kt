...
class KotlinCompilation internal constructor(
	...
	class Result internal constructor(
		...
		/**
		 * Steps can put data into result which is kept in this field.
		 * This allows them to provide additional outputs in the result
		 */
		 ...
		 */
		val generatedFiles: Collection<File> = sourcesGeneratedByAnnotationProcessor +
	...
	fun compile(): Result {
		...
		*/
		val exeuctionResult = withSystemProperty("idea.use.native.fs.for.win", "false") {
			runSteps(SingleCompilationModel(model))
		}
	...
	private fun makeResult(
		...
		executionResult.resultPayloads.forEach {
			it.outputDirs.forEach {
				it.copyRecursively(classesOutput)
			}
			it.generatedSourceDirs.forEach {
				it.copyRecursively(generatedSourcesOutput)
			}
		}
		return Result(
